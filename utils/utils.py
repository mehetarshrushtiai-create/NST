...
class ImageFolderDataset(Dataset):
    def __init__(self, root, transform = None):
        ...
        self.files = [p for p in self.files if p.lower().endswith(('.jpg', '.png', '.jpeg', '.webp'))]
...
def calc_mean_std(feat, eps=1e-5):
    ...
    feat_var=feat.view(batch_size, channel, -1).var(dim=2).view(batch_size, channel, 1, 1)+eps
